...
public class PdfUtil {
     ...
     */
    public static void exportZip(String templateName, String fileName, HttpServletResponse response, List<Map<String, Object>> listData) {
        try {
            fileName = URLEncoder.encode(fileName, "UTF-8");
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
     ...
     * @throws Exception
     */
    public static ByteArrayOutputStream generatePdfStream(String templateName, Map<String, Object> data) throws  Exception{
     ...
     * @return
     */
    public static Document makePdf(){
        Document doc = new Document(PageSize.A4);
        try {
            BaseFont bf = BaseFont.createFont("STSong-Light", "UniGB-UCS2-H", BaseFont.NOT_EMBEDDED);
            Font font = new Font(bf, 12, Font.NORMAL, BaseColor.BLACK);
            ...
            PdfPTable tbText = new PdfPTable(new float[] { 1f, 1.5f, 1f, 0.8f,1.6f, 3f});
            tbText.addCell(createCell("日期：", font, PdfPCell.ALIGN_LEFT, PdfPCell.ALIGN_LEFT, 0,0,15));
            //tbText.addCell(createCell("coamDutyDate",font,PdfPCell.ALIGN_LEFT, PdfPCell.ALIGN_MIDDLE, 0,0,15));
            tbText.addCell(createCell("班次：", font, PdfPCell.ALIGN_LEFT, PdfPCell.ALIGN_LEFT, 0,0,15));
            //tbText.addCell(createCell("coamClassName", font, PdfPCell.ALIGN_LEFT, PdfPCell.ALIGN_MIDDLE, 0,0,15));
            tbText.addCell(createCell("值守人员：", font, PdfPCell.ALIGN_LEFT, PdfPCell.ALIGN_LEFT, 0,0,15));
            //tbText.addCell(createCell("coamPesonName", font, PdfPCell.ALIGN_LEFT, PdfPCell.ALIGN_LEFT, 0,0,15));
            //增加空行
            tbText.addCell(createCell(" ",font,PdfPCell.ALIGN_LEFT, PdfPCell.ALIGN_LEFT,6,2,15));
            PdfPTable table = new PdfPTable(new float[] { 1f, 1.5f, 1f, 1.5f});
            //表头
